...
export function SidebarWidgets({ onOpenNewPlayer }: { onOpenNewPlayer: () => void }) {
  ...
  // Sort Leaderboard: Placed players first (by rank & CP), then Provisional players
  const sortedLeaderboard = [...(players || [])].sort((a, b) => {
    const aPlaced = a.placement_status === 'placed';
    const bPlaced = b.placement_status === 'placed';
          ...
          <table className="w-full text-left text-xs font-sans">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {sortedLeaderboard.length === 0 ? (
                ...
                sortedLeaderboard.slice(0, 8).map((player, idx) => {
                  ...
                  const isTop3 = idx < 3 && !isProvisional;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5">
                        {isProvisional ? (
                          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-teal-800 bg-teal-950/80 text-[#2dd4bf] font-bold">
                            Provisional
                          </span>
                        ) : (
                      ...
                      <td className="py-2.5 font-mono text-slate-200 font-bold group-hover:text-[#fbbf24]">
                        {isProvisional ? <span className="text-slate-500 font-normal">Calib.</span> : (player.current_cp || 0)}
                      </td>
                      <td className="py-2.5 font-mono text-[#fbbf24]">
                        {isProvisional ? (
                          <span className="text-[#2dd4bf] text-[11px]">{player.placement_matches_played || 0}/5 🎯</span>
                        ) : (
                          `${player.stars || 0}⭐`
                        )}
                      </td>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {matches.length === 0 ? (
                ...
                matches.slice(0, 5).map(m => {
                  ...
                  const padCourt = m.court_number ? String(m.court_number).padStart(2, '0') : 'PvP';
