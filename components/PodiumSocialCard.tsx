...
import { Crown, Sparkles, X, Share2, ShieldCheck } from 'lucide-react';
...
export function PodiumSocialCard({ onClose }: { onClose: () => void }) {
  ...
  const handleFinishAndArchive = () => {
    if (confirm("Conclude and archive today's session? Registered players and their updated Court Points remain permanently saved.")) {
      clearAllData();
      onClose();
        ...
        >
                  ...
                  <>
                    ...
                    <div className="space-y-0.5 w-full">
                      ...
                      <div className="text-[11px] font-mono font-black text-[#2dd4bf]">
                        {firstPlace.sessionNetCP >= 0 ? `+${firstPlace.sessionNetCP}` : firstPlace.sessionNetCP} CP Today
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-between text-[10px] font-mono text-slate-500 pt-2 border-t border-slate-800/80 relative z-10">
            <span className="flex items-center space-x-1">
              ...
              <span>OFFICIAL COURT POINTS (CP) RANKED SYSTEM</span>
            </span>
            <span>OPEN PLAY</span>
          </div>
        </div>
