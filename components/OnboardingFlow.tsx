...
import { GameMode, Gender } from '../types/pickleball';
import { ArrowRight, Plus, Check, Sparkles, Search, UserPlus, Shield } from 'lucide-react';
          ...
          <div className="bg-[#111c30] border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            {isCreatingNew && (
              <form onSubmit={handleCreateNewPlayer} className="p-4 bg-[#0b1220] border border-[#fbbf24]/40 rounded-2xl space-y-3">
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 rounded-xl bg-[#fbbf24] text-slate-950 font-bold text-xs cursor-pointer"
                >
                  Save to Directory (Provisional)
                </button>
              </form>
            )}
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              {filteredStoredPlayers.length === 0 ? (
                ...
                filteredStoredPlayers.map(p => {
                  ...
                  const isProvisional = p.placement_status === 'provisional';
                  const rankLabel = isProvisional 
                    ? `Provisional (${p.placement_matches_played || 0}/5)` 
                    : (p.rank_name || 'Wood III');
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`font-mono text-[11px] ${isProvisional ? 'text-[#2dd4bf] font-semibold' : 'text-slate-400'}`}>
                          • {rankLabel}
                        </span>
                      </div>
                      ...
                      {isRecruited ? (
                        <button
                          ...
                          className="px-3 py-1 rounded-lg bg-rose-950/80 text-[#f97316] border border-rose-800 font-mono text-[11px] font-bold cursor-pointer"
                        >
                          Remove ✕
                        </button>
                      ) : (
                        <button
                          ...
                          className="px-3 py-1 rounded-lg bg-[#fbbf24] hover:bg-[#f59e0b] text-slate-950 font-mono text-[11px] font-bold shadow-sm cursor-pointer"
                        >
                          + Recruit to Session
                        </button>
                      )}
        ...
        <div className="bg-[#111c30] border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-96 overflow-y-auto pr-1">
            {(checkins || []).map(chk => {
              ...
              const rankLabel = isProvisional 
                ? `Provisional (${player.placement_matches_played || 0}/5)`
              ...
              return (
                ...
                >
                  ...
                  <span className={`font-mono text-xs font-bold ${isProvisional ? 'text-[#2dd4bf]' : 'text-slate-300'}`}>
                    {rankLabel}
                  </span>
                </div>
              );
